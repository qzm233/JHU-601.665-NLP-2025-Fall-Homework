...
class Lexicon:
    ...
    def from_file(cls, file: Path) -> Lexicon:
        ...
        with open(file) as f:
            ...
            vocab_size, dim = map(int, first_line.strip().split())

            word2id = {}
            id2word = []
            embeddings = torch.zeros((vocab_size, dim), dtype=torch.float32)
            for idx, line in enumerate(f):  # All of the other lines are regular.
                ...
                word2id[word] = idx
                id2word.append(word)
                embeddings[idx] = torch.tensor(vec, dtype=torch.float32)

        lexicon = Lexicon()  # Maybe put args here. Maybe follow Builder pattern.
        lexicon.word2id = word2id
        lexicon.id2word = id2word
    ...
    def find_similar_words(
        ...
        if plus is not None and minus is not None:
            vec = vec + self.get_vector(plus) - self.get_vector(minus)
            banned.add(plus)
            banned.add(minus)

        vec = vec / torch.norm(vec) # ()
        lexicons_norms = torch.norm(self.embeddings, dim=1, keepdim=True) # satisfy broadcasting rules
        embeddings_normalized = self.embeddings / lexicons_norms    # (vocab_size, dim) / (vocab_size, 1) -> (vocab_size, dim)


        similarities = torch.matmul(embeddings_normalized, vec) 

        # Get the top 10 most similar words, excluding banned words.
        topk = 10 + len(banned)
        topk_indices = torch.topk(similarities, k=topk).indices.tolist()
        result = []
        for idx in topk_indices:
            candidate_word = self.id2word[idx]
            if candidate_word not in banned:
