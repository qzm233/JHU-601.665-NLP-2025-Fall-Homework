...
class HiddenMarkovModel:
    ...
    def setup_sentence(self, isent: IntegerizedSentence) -> None:
        """Precompute any quantities needed for forward/backward/Viterbi.
        Subclasses (like biRNN-CRF) can override this. Default: do nothing."""
        return
    ...
    def built_allowed(self,corpus):
        allowed = {} 
        for sentence in corpus:
            for word, tag in sentence:
                if tag is None or word in (BOS_WORD, EOS_WORD):
                    continue
                t_id = self.tagset.index(tag)
                if t_id is None:
                    continue
                w_id = self.vocab.index(word)  
                if w_id is None:
                    continue
                if w_id >= self.V:
                    continue
                allowed.setdefault(w_id, set()).add(t_id)
        inf = float('-inf')
        for w_id, tags in allowed.items():
            m = torch.full((self.k,), inf)
            for t in tags:
                m[t] = 0.0
            if torch.isneginf(m).all():
                m = torch.zeros(self.k)
            self.word_allowed_mask[w_id] = m
    ...
    def train(self,
        ...
        self.built_allowed(corpus=corpus)
    ...
    def forward_pass(self, isent: IntegerizedSentence) -> TorchScalar:
        ...
        n = len(isent)
        log_alpha = [torch.empty(self.k) for _ in range(n)] # a list of size [k] tensors
        log_alpha[0] = torch.log(self.eye[self.bos_t])  # size [k] tensor, 0 at BOS, -inf elsewhere; self.eye is an identical matrix

        # lA = torch.log(torch.clamp(self.A, min=1e-45)) # [k,k]
        # lB = torch.log(torch.clamp(self.B, min=1e-45)) # [n,k]
        
        for j in range(1, n):
            prev = log_alpha[j-1]                                 # [k]
            A = self.A_at(j, isent)              # [k, k]
            B = self.B_at(j, isent)              # [k, V]
            lA = torch.log(torch.clamp(A, min=1e-45))
            lB = torch.log(torch.clamp(B, min=1e-45))
            ...
            if j == n - 1:
                ...
                if self.awesome:
                    hard = self.word_allowed_mask.get(w_j, None)
                    if hard is not None and (isent[j][1] is None):
                        mask = mask + hard.to(mask.device, mask.dtype)
    ...
    def backward_pass(self, isent: IntegerizedSentence, mult: float = 1) -> TorchScalar:
        ...
        log_beta[-1] = torch.log(self.eye[self.eos_t])  # size [k] tensor, 0 at BOS, -inf elsewhere; self.eye is an identical matrix

        # lA = torch.log(torch.clamp(self.A, min=1e-45)) # [k,k]
        # lB = torch.log(torch.clamp(self.B, min=1e-45)) # [k,v]

        # go backward
        for j in range(n-2, -1, -1): # from n-2 to 0, ignore the last token EOS
            jp1 = j + 1
            A = self.A_at(jp1, isent)           # [k,k]
            B = self.B_at(jp1, isent)           # [k,V]
            lA = torch.log(torch.clamp(A, min=1e-45))
            lB = torch.log(torch.clamp(B, min=1e-45))
            
            if jp1 == n - 1: # EOS
                emit = torch.zeros(self.k)  # EOS no emission; in log-space it is add 0; in linear-space it is times 1
                mask = torch.full((self.k,), float('-inf')) # create a tensor with all position -inf
                mask[self.eos_t] = 0.0 # only eos visible
            else:
                w_jp1 = isent[jp1][0] # next token
                emit = lB[:, w_jp1] # [k], probability of emission to next token.
                if isent[jp1][1] is not None: # with tagging
                    t_jp1 = isent[jp1][1]
                    mask = torch.full((self.k,), float('-inf')); # [k,]
                    mask[t_jp1] = 0.0 # only next tagging visible
                else:
                    mask = torch.zeros(self.k) # all tagging visble
                if self.awesome:
                    hard = self.word_allowed_mask.get(w_jp1, None)
                    if hard is not None and isent[jp1][1] is None:
                        mask = mask + hard.to(mask.device, mask.dtype)
            nxt = log_beta[jp1] # [k]
            log_beta[j] = torch.logsumexp(lA + emit + mask + nxt, dim=1)  # pay attention to implicit broadcasting operation introduced by +

        self._log_beta = log_beta
        log_Z = self._log_Z  # from forward
        for j in range(1, n):
            A = self.A_at(j, isent)           # [k,k]
            B = self.B_at(j, isent)           # [k,V]
            lA = torch.log(torch.clamp(A, min=1e-45))
    ...
    def viterbi_tagging(self, sentence: Sentence, corpus: TaggedCorpus) -> Sentence:
        ...
        for j in range(1, n):
            ...
            lA = torch.log(torch.clamp(A, min=1e-45))
            lB = torch.log(torch.clamp(B, min=1e-45))
            # scores[s,t] = alpha[j-1][s] + lA[s,t]
            scores = alpha[j-1].view(-1, 1) + lA  # [k].view([k,1])+[k,k] = [k,k]

            if j == n - 1:
                emit = torch.zeros(self.k) # no emission
                mask = torch.full((self.k,), float('-inf'))
                mask[self.eos_t] = 0.0
            else:
                w_j = isent[j][0] # emission target token
                emit = lB[:, w_j] # [k], emission log-prob
                if isent[j][1] is not None:
                    t_j = isent[j][1]
                    mask = torch.full((self.k,), float('-inf'))
                    mask[t_j] = 0.0
            ...
            # alpha[j][t] = max_s scores[s,t] + emit[t] + mask[t]
            step_scores = scores + emit.view(1, -1) + mask.view(1, -1)  # [k,k] + [1,k] + [1,k] = [k,k]. The right position has been ready now, waiting to be taken out!
            ...
            if self.awesome:
                if j != n - 1:
                    ...
                    hard = self.word_allowed_mask.get(w_j, None)
                    if hard is not None and (isent[j][1] is None):
                        step_scores = step_scores + hard.to(step_scores.device, step_scores.dtype).view(1, -1)

            alpha[j], backpointers[j] = step_scores.max(dim=0)          # over s -> best s for each t

        tags: List[int] = [0] * n
        tags[-1] = self.eos_t
        for j in range(n - 1, 0, -1):
            prev = backpointers[j][tags[j]].item()
            tags[j - 1] = prev
    ...
    def posterior_decode(self, sentence: Sentence, corpus: TaggedCorpus) -> Sentence:
        """
        Per-position posterior decoding:
        tags[j] = argmax_t P(t_j = t | w_1:n)
        May produce an illegal path (e.g., disallowed transitions).
        """
        isent = self._integerize_sentence(sentence, corpus)
        n = len(isent)

        # 1) forward/backward to get log_alpha, log_beta, log_Z
        log_Z = self.forward_pass(isent)          # sets self._log_alpha, self._log_Z
        log_Z_bwd = self.backward_pass(isent)     # sets beta internally (if you cached), or just returns
        assert torch.isclose(log_Z, log_Z_bwd)

        log_alpha = self._log_alpha
        log_beta  = self._log_beta

        tags: List[int] = [self.bos_t] + [0]*(n-2) + [self.eos_t]
        for j in range(1, n-1):  # 非BOS/非EOS
            log_gamma = log_alpha[j] + log_beta[j] - log_Z

            if isent[j][1] is not None:
                t_j = isent[j][1]
                mask = torch.full((self.k,), float('-inf'))
                mask[t_j] = 0.0
                log_gamma = log_gamma + mask

            w_j = isent[j][0]
            
            if self.awesome:
                hard = getattr(self, "word_allowed_mask", {}).get(w_j, None)
                if hard is not None and isent[j][1] is None:
                    log_gamma = log_gamma + hard.to(log_gamma.device, log_gamma.dtype)

            tags[j] = int(torch.argmax(log_gamma).item())
        tags[0]   = self.bos_t
        tags[-1]  = self.eos_t

        return Sentence([(word, self.tagset[tags[j]]) for j, (word, _tag) in enumerate(sentence)])
